...
**PostgreSQL en lugar de SQL Server.** El enunciado permite ambos. PostgreSQL además ofrece índices
únicos parciales, que resuelven de forma limpia la convivencia entre unicidad y eliminación lógica.

**Modelo de dominio rico.** `Pedido` no expone setters públicos: se crea con `Pedido.Crear(...)` y se
modifica con métodos que validan sus invariantes. Es imposible construir un pedido inválido desde
ninguna capa.

**Un caso de uso por clase.** Cada operación es una clase con su interfaz. Más ceremonia que un
servicio con cinco métodos, pero cada clase tiene una sola razón para cambiar y los controllers
dependen de abstracciones.

**Sin MediatR ni AutoMapper.** Ambos pasaron a licencia comercial. El despacho lo hace el contenedor
de inyección de dependencias y el mapeo es un método de extensión de pocas líneas.

**Sin librería de validación adicional.** Las reglas ya viven en el dominio; duplicarlas en un
validador crearía dos fuentes de verdad. La capa de aplicación solo valida lo que el dominio no puede
saber, como la unicidad del número de pedido.

**Doble defensa ante duplicados.** El caso de uso consulta antes de guardar, y la unidad de trabajo
traduce el `SQLSTATE 23505` de PostgreSQL a la misma excepción de dominio. Sin lo segundo, dos
peticiones simultáneas pasarían ambas la verificación previa.

**Sin `try/catch` en los controllers.** Un `IExceptionHandler` global traduce cada excepción de
dominio a su código HTTP en un único lugar.
